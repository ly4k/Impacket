...
def computeResponseNTLMv2(flags, serverChallenge, clientChallenge, serverName, domain, user, password, lmhash='',
    ...
    if password == "CHALLENGE":
        print("Challenge: %s:%s:%s:%s" % (domain.encode("utf-16le").hex(), user.encode("utf-16le").hex(), serverName.hex(), serverChallenge.hex()))
        response = input("Enter Response: ").split(":")
        sessionBaseKey = bytes.fromhex(response[0])
        ntChallengeResponse = bytes.fromhex(response[1])
        return ntChallengeResponse, b"", sessionBaseKey
